...
func asyncHandleNotification(dic *di.Container, n models.Notification, sub models.Subscription, address models.Address) {
	...
	if err != nil {
		lc.Warnf("fail to send notification to the subscription %s with address %v, err: %v", sub.Name, address.GetBaseAddress(), err)
		return
	}
	...
	if n.Severity == models.Critical && trans.Status == models.Failed {
		trans, err = criticalSend(dic, n, trans)
		if err != nil {
			lc.Warnf("fail to send the critical notification to the subscription %s with address %v, err: %v", sub.Name, address.GetBaseAddress(), err)
			return
		}
	}
	...
	if trans.Status == models.Escalated {
		...
		if err != nil {
			lc.Warnf("fail to send the escalated notification, err: %v", err)
		}
	}
}
