...
func AllReadings(parms query.Parameters, dic *di.Container) (readings []dtos.BaseReading, totalCount uint32, err errors.EdgeX) {
	...
	processReadingDTOs(parms, readings)
...
func processReadingDTOs(parms query.Parameters, readings []dtos.BaseReading) {
	for i, reading := range readings {
		// simple reading to numeric reading
		if parms.Numeric && len(reading.SimpleReading.Value) != 0 {
			switch reading.ValueType {
			case common.ValueTypeInt8, common.ValueTypeInt16, common.ValueTypeInt32, common.ValueTypeInt64:
				readings[i].NumericReading.Value = cast.ToInt64(reading.SimpleReading.Value)
			case common.ValueTypeUint8, common.ValueTypeUint16, common.ValueTypeUint32, common.ValueTypeUint64:
				readings[i].NumericReading.Value = cast.ToUint64(reading.SimpleReading.Value)
			case common.ValueTypeFloat32, common.ValueTypeFloat64:
				readings[i].NumericReading.Value = cast.ToFloat64(reading.SimpleReading.Value)
			}
			readings[i].SimpleReading.Value = ""
		}
		// numeric reading to simple reading
		if !parms.Numeric && reading.NumericReading.Value != nil {
			readings[i].SimpleReading.Value = cast.ToString(reading.NumericReading.Value)
			reading.NumericReading.Value = nil
		}
	}
}
